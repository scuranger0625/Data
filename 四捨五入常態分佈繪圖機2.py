...
def plot_normal_distribution(mean, std_dev, value):
    ...
    # 在圖表上標註每個標準差的值
    for i in range(-3, 4):
        plt.text(mean + i*std_dev, 0, '{:.2f}'.format(mean + i*std_dev), verticalalignment='bottom', horizontalalignment='center', fontsize=10)

    # 添加顯示平均數的線
    plt.axvline(mean, color='orange', linestyle='--', label='Mean')

    # 添加顯示標準差的線
    plt.axvline(mean + std_dev, color='gray', linestyle='--', label='1σ 68.2%')
    plt.axvline(mean - std_dev, color='gray', linestyle='--')
    plt.axvline(mean + 2*std_dev, color='green', linestyle='--', label='2σ 95%')
    plt.axvline(mean - 2*std_dev, color='green', linestyle='--')
    plt.axvline(mean + 3*std_dev, color='blue', linestyle='--', label='3σ 99.7%')
...
def main():
    ...
    # 輸出結果
    print("給定值的 Z 分數為：{:.2f}".format(z_score))
